...
durgesh_ai_source = os.getenv("DURGESH_AI_SOURCE", os.path.abspath(os.path.join(os.path.dirname(__file__), "..")))
system = os.environ.get('DURGESH_AI_PLATFORM', platform.system())
architecture = os.environ.get('DURGESH_AI_ARCHITECTURE', platform.architecture()[0])

# Copy precompmilled libraries
for lib in glob.glob(os.path.join(durgesh_ai_source, "src_durgesh/lib*.*")):
    print ("Adding library", lib)
    shutil.copy(lib, "durgesh_ai")
...
setuptools.setup(
    ...
    description="Offline open source speech recognition API based on Kaldi",
    long_description=long_description,
    long_description_content_type="text/markdown",
    url="",
    packages=setuptools.find_packages(),
    package_data = {'durgesh_ai': ['*.so', '*.dll', '*.dyld']},
    ...
    cffi_modules=['durgesh_ai_builder.py:ffibuilder'],
)
